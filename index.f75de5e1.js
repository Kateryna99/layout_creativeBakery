...
function toggleComponents(burger, navHeader) {
    ...
    if (navHeader.classList.contains("nav-header--active")) document.body.style.overflow = "hidden";
...
function closeMenuOnMenuLinkClick(navHeaderLinks, burger, navHeader) {
    for (let item of navHeaderLinks)item.addEventListener("click", ()=>toggleComponents(burger, navHeader));
}
